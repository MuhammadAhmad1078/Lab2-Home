...
const TestBookingForm: React.FC<TestBookingFormProps> = ({ selectedLab }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        const intentResponse = await paymentAPI.createPaymentIntent(response.data._id);
        if (intentResponse.success) {
          setClientSecret(intentResponse.clientSecret);
          setPaymentStep(true);
        } else {
          toast.error("Booking created but payment gateway failed. You can pay at the lab.");
          resetForm();
        }
      } else {
  ...
  const handlePaymentSuccess = () => {
    toast.success("Payment Received! Your booking is fully confirmed.");
    resetForm();
  };
